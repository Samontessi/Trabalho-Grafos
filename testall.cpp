...
public:
    ...
    // --- Informações sobre vértices: EQUIPE DE OPERAÇÕES (a implementar) ---
    // TODO(operações): ligar estes métodos à implementação real (grau, vizinhos,
    // adjacência). Enquanto não forem feitos, os testes correspondentes do
    // testAll() vão FALHAR de propósito, sem quebrar a execução.
    int grauVertice(int v) { return 0; }
    std::vector<int> listarVizinhos(int v) { return {}; }
    bool saoAdjacentes(int u, int v) { return false; }
};
